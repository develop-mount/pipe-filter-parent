 ...
 */
@Slf4j
public class ReplaceRegexFilter extends BasePipeFilter<Object, Object> {
    ...
    public PipeDataWrapper<Object> handlerApply(PipeDataWrapper<Object> wrapper) {
        ...
        if (PipeFilterUtils.isEmpty(params()) || params().size() != PARAMS_NUM) {
            return PipeDataWrapper.error(errorPrefix() + "[replace-regex:oldChar,newChar,all]传入参数下标为空或是超过两个");
        }

        String oldChar = params().get(0);
        if (StringUtils.isBlank(oldChar)) {
            return PipeDataWrapper.error(errorPrefix() + "[replace-regex:oldChar,newChar,all]中传入参数oldChar为空");
        }

        String newChar = params().get(1);
        if (StringUtils.isBlank(newChar)) {
            return PipeDataWrapper.error(errorPrefix() + "[replace-regex:oldChar,newChar,all]中传入参数newChar为空");
        }

        String all = params().get(2);
        if (StringUtils.isBlank(all)) {
            return PipeDataWrapper.error(errorPrefix() + "[replace-regex:oldChar,newChar,all]中传入参数all为空,不是1/0");
        }
        if (!StringUtils.isNumeric(all)) {
            return PipeDataWrapper.error(errorPrefix() + "[replace-regex:oldChar,newChar,all]中传入参数all错误,不是1/0");
        }
        ...
        try {
            ...
            if (1 == allInt) {
                return PipeDataWrapper.success(((String) value).replaceAll(oldCharEs, newCharEs));
            } else {
